...
const ProfilePhotoSelector = ({image, setImage}) => {
    ...
    return (
        <div className="flex justify-center mb-6">
            ...
            {!image ? (
                <div className="w-20 h-20 flex items-center justify-center bg-purple-200 text-white shadow-inner rounded-[10px] rounded-full relative">
                    <User className="text-purple-500" size={35} />

                    <button onClick={onChooseFile}
                    className="w-8 h-8 flex items-center justify-center bg-primary text-purple-900 rounded-full absolute -bottom-1 -right-1">
                    <Upload size={15} />
                    </button>
                </div>
            ) :(
